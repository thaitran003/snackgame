...
class Snake:
	...
	def update(self):
		if not self.has_eaten:
			# Nếu rắn chưa ăn quả táo:
			# - Sao chép danh sách các phần của rắn trừ phần cuối (đuôi).
			body_copy = self.body[:-1]
			# - Chèn phần mới vào vị trí đầu tiên của danh sách dựa trên hướng di chuyển.
			body_copy.insert(0, body_copy[0] + self.direction)
			# - Cập nhật danh sách phần của rắn.
			self.body = body_copy[:]
		else:
			# Nếu rắn đã ăn quả táo:
			# - Sao chép danh sách các phần của rắn.
			body_copy = self.body[:]
			# - Chèn phần mới vào vị trí đầu tiên của danh sách dựa trên hướng di chuyển.
			body_copy.insert(0, body_copy[0] + self.direction)
			# - Cập nhật danh sách phần của rắn.
			self.body = body_copy[:]
			# - Đặt lại trạng thái đã ăn quả táo về False để chuẩn bị cho lần tiếp theo.
			self.has_eaten = False
	...
	# Thiết lập lại trạng thái ban đầu của rắn khi kết thúc trò chơi
	def reset(self):
		self.body = [pygame.Vector2(START_COL - col,START_ROW) for col in range(START_LENGTH)]
		self.direction = pygame.Vector2(1,0)
	...
	# Cập nhật hình ảnh đầu của rắn dựa trên hướng di chuyển
	...
	# Cập nhật hình ảnh đuôi của rắn dựa trên hướng di chuyển
	...
	# Cập nhật hình ảnh các phần cơ thể của rắn
